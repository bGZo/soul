...
- ## 绪论
  ...
  - Data #vs DB #vs DBMS #vs DBS
    collapsed:: true
    - 数据 (Data)
      mark:: "描述事物的符号记录，是数据库中的基本对象"
    - 数据库 (Database)
      mark:: 是长期储存在计算机内、有组织的、可共享的大量数据的集合 数据库
    - 数据库管理系统 (DBMS, Database Management System)
      mark:: 是位于用户与操作系统之间的一层数据管理软件
      - 主要功能
        - 数据定义功能
        - 数据组织、存储和管理
        - 数据操纵功能(4)
        ...
        - 其他功能
    - 数据库系统 (DBS)
      mark:: 由数据库、数据库管理系统 (及其开发工具) 、应用系统、数据库管理员构成的存储、管理、处理和维护数据的系统.
  -
  - 数据管理阶段
    collapsed:: true
    - 人工管理阶段
...
- ## 关系数据库
  ...
  - 概念
    - 数据的完整性约束条件
      mark:: 是指给定的数据模型中数据及其联系所具有的制约和依存规则
    - 实体
      mark:: 客观存在并可相互区别的事物
    - 属性
      mark:: 实体所具有的某一特性
    ...
    - 域
      mark:: 属性的取值范围
    - 实体型
      mark:: "具有相同属性的实体必然具有的特征和性质. 用实体名及其属性名集合来抽象和刻画同类实体，成为实体型"
    - 实体集
      mark:: 同一类型实体的集合
    - 联系
      mark:: 实体内部的联系通常是指组成实体的各属性之间的联系；实体之间的联系通常是指不同实体集之间的联系. 分为三种
      - 一对一联系
      - 一对多联系
      - 多对多联系
    - 关系模型
    ...
    - 属性
      mark:: 表中的一列即为一个属性
    - 码 (码键)
      mark:: "表中的某个属性组，它可以唯一确定一个元组"
    - 域
      mark:: 属性的取值范围
    - 分量
      mark:: 元组中的一个属性值
    - 候选码
      mark:: 若关系中的某一属性组的值能唯一的标识一个元组
    - 主码
      mark:: "若一个关系中有多个候选码，则选定其中一个为主码."
    - 主属性
      mark:: 候选码的诸属性. 不包含在任何候选码中的属性称为**非主属性或非码属性**
  - 关系的三种类型
    - 基本关系 (又称基本表、基表)
    - 查询表
    - 视图表
...
- ## 数据库设计
  ...
  - 数据库设计
    mark:: "对于一个给定的应用环境，构造 (设计) 优化的数据库逻辑模式和物理结构，并据此建立数据库及应用系统，使之能够有效的存储和管理数据，满足各种用户的应用需求，包括信息管理要求和数据操作要求"
  ...
  - E-R 图
    ...
    mark:: Entity Relationship Diagram. 实体-联系图. 提供了表示**实体类型、属性和联系的方法**. 用来描述现实世界的概念模型. 用来捋清数据库设计
...
- ## 数据库恢复技术  #transaction
  - 事务 
    mark:: "用户定义的一个数据库操作序列，这些操作要么全做要么全不做，是不可分割的工作单位"
  ...
  - **事务隔离级别**
    ...
    mark:: **定义了数据库系统中一个事务中操作的结果在何时以何种方式对其他并发事务操作可见**
    source:: [Wikipedia](https://en.wikipedia.org/wiki/Isolation_(database_systems))
